...
pycrypto = Tk()
pycrypto.title("My Crypto Portfolio") #.title is for title
pycrypto.iconbitmap("favicon.ico")#.iconbitmap is a function used for changing the icon
...
def my_portfolio():
    ...
    def font_color(amount):#the colour chnging is for profit and loss
        if amount >= 0:
            return "green"
        else:
            return "red"
    ...
    for i in range(0, 300):
      for coin in coins:
        if api["data"][i]["symbol"] == coin[1]:
          ...
          portfolio_id.grid(row=coin_row, column=0, sticky=N+S+E+W)#label function is used for putting a information about a particular block
          #.grid is used to specify the plce of block 
